...
local function Hide(Interface, JustHide: boolean?, Notify: boolean?, Bind: string?)
	...
	if Interface.ClassName ~= "ScreenGui" then
		...
		-- Actually Hide The Stuff (apply instantly)
		local iid = nil
		pcall(function()
			iid = Interface:GetAttribute("InstanceID")
		end)
		local entry = TransparencyValues[Interface.Name] and TransparencyValues[Interface.Name][iid]
		if Interface.ClassName == "Frame" then
			pcall(function()
	...
	-- Immediately disable visibility instead of waiting for tweens
	if Interface.ClassName == "ScreenGui" then
	...
	pcall(function()
		if Interface.ClassName ~= "ScreenGui" then
			local ok, iid = pcall(function()
				return Interface:GetAttribute("InstanceID")
			end)
			if ok and iid then
				local entry = TransparencyValues[Interface.Name] and TransparencyValues[Interface.Name][iid]
				if type(entry) == "table" then
					if Interface.ClassName == "Frame" and entry.BackgroundTransparency ~= nil then
						pcall(function()
							Interface.BackgroundTransparency = entry.BackgroundTransparency
						end)
					end
					...
					then
						pcall(function()
							Interface.BackgroundTransparency = entry.BackgroundTransparency
							Interface.TextTransparency = entry.TextTransparency
						end)
					end
					...
					then
						pcall(function()
							Interface.BackgroundTransparency = entry.BackgroundTransparency
							Interface.ImageTransparency = entry.ImageTransparency
						end)
					end

					if
						(Interface.ClassName == "UIStroke" or Interface.ClassName == "UIGradient")
						and entry.Transparency ~= nil
					then
						pcall(function()
							Interface.Transparency = entry.Transparency
						end)
					end
				end
			end
...
function Starlight:CreateWindow(WindowSettings)
	...
	function Starlight.Window:PromptDialog(ModalSettings)
		...
		-- Replace UIListLayout with UIGridLayout for button grid
		if Modal.Instance.Holder.Actions:FindFirstChild("UIListLayout") then
			Modal.Instance.Holder.Actions:FindFirstChild("UIListLayout"):Destroy()
		end
		local GridLayout = Instance.new("UIGridLayout")
		GridLayout.Parent = Modal.Instance.Holder.Actions
		GridLayout.CellSize = UDim2.fromOffset(120, 40)
		...
		if Modal.Values.Type == 1 then
			...
			-- Type 2: Input - restore UIListLayout for vertical arrangement
			if Modal.Instance.Holder.Actions:FindFirstChild("UIGridLayout") then
				Modal.Instance.Holder.Actions:FindFirstChild("UIGridLayout"):Destroy()
			end
			local ListLayout = Instance.new("UIListLayout")
			ListLayout.Parent = Modal.Instance.Holder.Actions
	...
	function Starlight.Window:CreateHomeTab(TabSettings)
		...
		Tab.Instances.Page.Holder.Left.Discord.Interact.MouseButton1Click:Connect(function()
			...
			Starlight:Notification({ Title = "Copied", Content = "Discord invite copied to clipboard!", Duration = 3 })
			if Request then
				pcall(function()
					Request({
